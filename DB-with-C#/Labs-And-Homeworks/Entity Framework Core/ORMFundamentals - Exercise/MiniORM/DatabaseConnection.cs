	...
	internal class DatabaseConnection
	{
			...
			where T : class
		{
			...
			foreach (var entity in entitiesToDelete)
			{
				...
				var deletedRows = this.ExecuteNonQuery(query, primaryKeyParameters);
		...
		private static T MapColumnsToObject<T>(string[] columnNames, object[] columnsValues)
		{
			...
			for (var i = 0; i < columnsValues.Length; i++)
			{
				...
				var columnValue = columnsValues[i];
