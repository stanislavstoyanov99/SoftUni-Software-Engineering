namespace PetStore.Services.Implementations
{
    ...
    public class BrandService : IBrandService
    {
        ...
        public int Create(string name)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(ExceptionMessages.InvalidBrandName,
                    DataValidation.NameMaxLength));
            }

            if (this.data.Brands.Any(br => br.Name == name))
            {
                throw new InvalidOperationException(string.Format(ExceptionMessages.BrandNameAlreadyExists,
                    name));
            }
